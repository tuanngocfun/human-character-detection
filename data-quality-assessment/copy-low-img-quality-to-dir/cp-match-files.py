...
for entry in json_data:
    ...
    if image_id is not None and mean_score_prediction is not None:
        ...
        if mean_score_prediction < 5:
            image_id_number = int(image_id)
            next_image_id = str(image_id_number + 1).zfill(5) + ".jpg"

            source_image_path = os.path.join(image_dir_path, next_image_id)
            target_image_path = os.path.join(target_dir_path, next_image_id)

            # Check if the corresponding image exists and copy it to the target directory
            if os.path.exists(source_image_path):
                shutil.copy(source_image_path, target_image_path)
                print(f"Copied {next_image_id} to {target_dir_path}")
