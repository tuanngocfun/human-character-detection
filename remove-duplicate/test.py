...
source_dir = '../../test-data/complete-first-w-human-real-world-iou-95-70-conf-animation-conf90-iou95/images'
target_dir = '../../test-data/complete-first-w-human-real-world-iou-95-70-conf-animation-conf90-iou95/similarity-images'
threshold = 0.90
image_size = (100, 100)
max_workers = 24
...
for i, similar_indices in enumerate((similarities >= threshold) & ~processed_images):
    ...
    # Skip if it's a single image (itself)
    if len(group) == 1:
        continue
    ...
    # Move only the images that are NOT the first in the group
    for j in group[1:]:
